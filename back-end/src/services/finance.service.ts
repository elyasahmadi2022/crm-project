...
export const financeService = {
    ...
    async getInvoiceProfile(id: number): Promise<InvoiceResponseDto> {
        const invoice = await financeRepository.findInvoiceWithDetails(id);
        if (!invoice) throw new AppError(404, "Invoice not found.");
        return toInvoiceResponseDto(invoice);
    },

    async getAllInvoices(filters: ListInvoicesQueryDto, page = 1, limit = 10) {
    ...
    async updateInvoice(id: number, data: UpdateInvoiceDto): Promise<InvoiceResponseDto> {
        const current = await financeRepository.findInvoiceById(id);
        if (!current) throw new AppError(404, "Invoice not found.");
        const updated = await financeRepository.updateInvoice(id, data as unknown as Prisma.InvoiceUpdateInput);
        return toInvoiceResponseDto(updated);
    },

    async changeInvoiceStatus(id: number, newStatus: InvoiceStatus, userId: number): Promise<InvoiceResponseDto> {
        const current = await financeRepository.findInvoiceById(id);
        if (!current) throw new AppError(404, "Invoice not found.");
        if (current.status === newStatus) return this.getInvoiceProfile(id);
    ...
    async addPayment(invoiceId: number, data: CreatePaymentDto): Promise<PaymentResponseDto> {
    ...
    async getBudgetProfile(id: number): Promise<BudgetResponseDto> {
    ...
    async createBudget(data: CreateBudgetDto): Promise<BudgetResponseDto> {
        const budget = await financeRepository.createBudget(data as unknown as Prisma.BudgetCreateInput);
        return toBudgetResponseDto(budget);
    },

    async updateBudget(id: number, data: UpdateBudgetDto): Promise<BudgetResponseDto> {
        // Existence check added — previously missing
        const current = await financeRepository.findBudgetById(id);
        if (!current) throw new AppError(404, "Budget not found.");
        const updated = await financeRepository.updateBudget(id, data as unknown as Prisma.BudgetUpdateInput);
    ...
    async getExpenseProfile(id: number): Promise<ExpenseResponseDto> {
        const expense = await financeRepository.findExpenseById(id);
        if (!expense) throw new AppError(404, "Expense not found.");
        return toExpenseResponseDto(expense);
    },

    async getAllExpenses(filters: ListExpensesQueryDto, page = 1, limit = 10) {
        const skip = (page - 1) * limit;
        const whereClause: Prisma.ExpenseWhereInput = {};
        if (filters.category) whereClause.category = filters.category;
        if (filters.budgetId) whereClause.budgetId = filters.budgetId;
    ...
    async updateExpense(id: number, data: UpdateExpenseDto): Promise<ExpenseResponseDto> {
        const current = await financeRepository.findExpenseById(id);
        if (!current) throw new AppError(404, "Expense not found.");
