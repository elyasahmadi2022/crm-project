...
if (!IS_DEV) {
    if (!MAIL_HOST) throw new Error("Missing required env var: MAIL_HOST");
    if (!MAIL_USER) throw new Error("Missing required env var: MAIL_USER");
    if (!MAIL_PASS) throw new Error("Missing required env var: MAIL_PASS");
}
...
export const emailService = {
     ...
     */
    sendOtpEmail: async (toEmail: string, userName: string, otp: string): Promise<void> => {
