...
export const otpService = {
     ...
     */
    generateAndStore: async (userId: number): Promise<string> => {
     ...
     */
    verify: async (userId: number, plainCode: string): Promise<void> => {
     ...
     */
    invalidateAll: async (userId: number): Promise<void> => {
        await prisma.otpCode.updateMany({
            where: { userId, purpose: OTP_PURPOSE_RESET, usedAt: null },
            data: { usedAt: new Date() },
        });
