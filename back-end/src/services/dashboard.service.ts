...
export const DashboardService = {
  async getOverview() {
    ...
    const activeProjectsCount = projects.filter(p => 
      ['REQUIREMENTS', 'DESIGN', 'DEVELOPMENT', 'TESTING', 'DEPLOYMENT'].includes(p.stage)
    ).length;
    ...
    const openLeadsCount = leads.filter(l => 
      ['NEW', 'CONTACTED', 'PENDING'].includes(l.status)
    ).length;
    ...
    const thisMonthInvoices = invoices.filter(inv => 
      inv.issueDate && inv.issueDate >= thisMonthStart
    );
    ...
    const formattedProjects = projects.slice(0, 5).map(p => {
      ...
      let updatedStr = '';
      if (updatedDiff < 1) updatedStr = 'Just now';
      else if (updatedDiff < 24) updatedStr = `${updatedDiff}h ago`;
      else updatedStr = `${Math.floor(updatedDiff / 24)}d ago`;
    ...
    const totalsByCurrency = invoices.reduce((totals, inv) => {
      ...
      if (isOverdue) current.overdue += balance;
      else current.outstanding += balance;
      totals[inv.currency] = current;
      return totals;
    }, {} as Record<string, { invoiced: number; paid: number; outstanding: number; overdue: number }>);
