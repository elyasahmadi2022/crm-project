...
export class PayrollService {
  ...
  async updatePayroll(id: number, dto: UpdatePayrollDto) {
  ...
  async payPayroll(id: number, dto: PayPayrollDto) {
    return prisma.$transaction(async (tx) => {
      ...
      if (salaryAmount <= 0 || !Number.isFinite(salaryAmount)) throw new Error("Salary payment amount must be greater than zero")
      if (salaryAmount > remainingSalary + 0.0001) throw new Error(`Payment exceeds the remaining salary of ${remainingSalary.toFixed(2)} ${payroll.salaryCurrency}`)
      ...
      const account = await tx.account.findUnique({
        ...
        if (!Number.isFinite(exchangeRate) || exchangeRate <= 0) throw new Error("Exchange rate must be greater than zero")
  ...
  async recordAdvance(dto: RecordAdvanceDto) {
    const employee = await prisma.user.findUnique({ where: { id: dto.employeeId }, select: { salaryCurrency: true } })
    if (!employee) throw new Error("Employee not found")
  ...
  async listAdvances(employeeId?: number) {
  ...
  async deductAdvance(dto: DeductAdvanceDto) {
    const advance = await prisma.payrollAdvance.findUnique({
  ...
  async generateMonthlyPayroll(dto: GenerateMonthlyPayrollDto) {
    ...
    const payrollRecords = employees.map((emp) => {
      ...
      const totalAdvances = empAdvances.reduce(
        (sum, a) => sum + (Number(a.amount) - Number(a.deductedAmount)),
        0
      )
    ...
    return prisma.$transaction(
      payrollRecords.map((record) => prisma.payroll.create({ data: record }))
    )
  }
  ...
  async getMonthlyReport(month: number, year: number) {
    ...
    const totalBaseSalary = payrolls.reduce(
      (sum, p) => sum + Number(p.baseSalary),
      0
    )
    const totalAdvances = payrolls.reduce(
      (sum, p) => sum + Number(p.advances),
      0
    )
    const totalDeductions = payrolls.reduce(
      (sum, p) => sum + Number(p.deductions),
      0
    )
  ...
  async getEmployeePayrollHistory(employeeId: number, limit = 12) {
    return prisma.payroll.findMany({
      where: { employeeId },
      include: {
        paidFrom: true,
