...
const parseId = (p: string | string[] | undefined) =>
  parseInt(Array.isArray(p) ? (p[0] ?? '0') : (p ?? '0'), 10);
...
export const contractTemplateController = {
  ...
  getDefault: async (_req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  update: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const dto = Object.fromEntries(Object.entries(raw).filter(([, v]) => v !== undefined)) as typeof raw;
  ...
  saveLayout: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const id = parseId(req.params.id);
      const { layoutJson } = req.body as { layoutJson?: string };
      ...
      let parsed: unknown;
      try { parsed = JSON.parse(layoutJson); } catch {
