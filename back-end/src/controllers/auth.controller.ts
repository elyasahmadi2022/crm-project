...
const formatZodIssues = (issues: ZodIssue[]): string[] =>
    issues.map((e) => (e.path.length ? `${e.path.map(String).join(".")}: ${e.message}` : e.message));
...
export const authController = {
    ...
    refresh: asyncHandler(async (req: Request, res: Response): Promise<void> => {
        ...
        if (!tokenFromCookie) throw new AppError(401, "No refresh token found. Please log in again.");
