...
import {
    createInvoiceSchema, updateInvoiceSchema, listInvoicesQuerySchema, changeInvoiceStatusSchema,
    createPaymentSchema, createBudgetSchema, updateBudgetSchema,
...
export const financeController = {
    ...
    changeInvoiceStatus: async (req: Request, res: Response, next: NextFunction) => {
        try {
            const id = parseInt(req.params.id as string, 10);
    ...
    // --- Budgets ---
    getAllBudgets: async (req: Request, res: Response, next: NextFunction) => {
        try {
            const { page, limit } = paginationSchema.parse(req.query);
