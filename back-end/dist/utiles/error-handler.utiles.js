...
export const errorHandler = (err, _req, res, _next) => {
    ...
    if (err instanceof ZodError) {
        const messages = err.issues.map((e) => e.path.length ? `${e.path.map(String).join(".")}: ${e.message}` : e.message);
    ...
    if (err instanceof Prisma.PrismaClientValidationError) {
        ...
        return sendError(res, 500, "A server configuration error occurred. Please contact support.");
    }
...
//# sourceMappingURL=error-handler.utiles.js.map