import { toCompanySettingsDto, toContractTemplateDto, } from '../dtos/company.dto.js';
...
export const contractTemplateService = {
    ...
    async getById(id) {
        ...
        if (!t)
            throw new AppError(404, 'Contract template not found.');
    ...
    async update(id, dto) {
        ...
        if (dto.isDefault === true && !existing.isDefault) {
            ...
            if (!refreshed)
                throw new AppError(404, 'Contract template not found after update.');
            // Apply any other field changes on top
            const rest = Object.fromEntries(Object.entries(dto).filter(([k, v]) => k !== 'isDefault' && v !== undefined));
