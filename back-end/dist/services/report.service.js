...
export class ReportService {
    ...
    async createReport(dto) {
        ...
        const limits = { DAILY: 1, WEEKLY: 2, MONTHLY: 3 };
        ...
        if (existingCount >= limit) {
            ...
            throw new Error(`You can only submit ${max} of type "${labels[dto.type] ?? dto.type}" per day. ` +
    ...
    // ────────────────────────────────────────────────────────────────────
    async getWeeklySummary(employeeId, year, weekNumber) {
        ...
        const missingWeekdays = [1, 2, 3, 4, 5, 6, 0].filter((day) => !reportedWeekdays.has(day));
    ...
    async getMonthlySummary(employeeId, year, month) {
        ...
        const effectiveStartDay = joinDay && joinDay > monthStart
        ...
        // Get reported dates
        const reportedDates = new Set(reports.map((r) => r.reportDate.toISOString().split("T")[0]));
    ...
    // ────────────────────────────────────────────────────────────────────
    async getAllEmployeesWeeklySummary(year, weekNumber) {
        ...
        const summaries = await Promise.all(employees.map((emp) => this.getWeeklySummary(emp.id, year, weekNumber)));
        return summaries.map((summary, idx) => ({
            ...employees[idx],
            ...summary,
        }));
    }
    async getAllEmployeesMonthlySummary(year, month) {
        const employees = await prisma.user.findMany({
            where: {
                isActive: true,
                role: { not: "ADMIN" },
        ...
        const summaries = await Promise.all(employees.map((emp) => this.getMonthlySummary(emp.id, year, month)));
        return summaries.map((summary, idx) => ({
            ...employees[idx],
            ...summary,
        }));
