...
export class AttendanceService {
    uploadDir = process.env.UPLOAD_DIR || "uploads/faces";
    // ────────────────────────────────────────────────────────────────────
    // Face Recognition
    // ────────────────────────────────────────────────────────────────────
    async registerFace(dto) {
        ...
        for (const other of others) {
            if (!other.faceEmbedding)
                continue;
            const stored = JSON.parse(other.faceEmbedding);
            if (stored.length !== 128)
                continue;
            const distance = this.euclideanDistance(stored, dto.faceDescriptor);
            if (distance < DUPLICATE_THRESHOLD) {
                throw new Error(`This face is already registered to another employee (${other.name}). Each employee must register their own face.`);
            }
        }
        // ─────────────────────────────────────────────────────────────────
    ...
    async verifyFace(dto) {
        ...
        for (const emp of employees) {
            if (!emp.faceEmbedding)
                continue;
            const stored = JSON.parse(emp.faceEmbedding);
            if (stored.length !== 128)
                continue;
            const distance = this.euclideanDistance(stored, dto.faceDescriptor);
            candidates.push({
    ...
    async verifyFaceForEmployee(dto) {
        const employee = await prisma.user.findUnique({
            where: { id: dto.employeeId },
        ...
        const storedDescriptor = JSON.parse(employee.faceEmbedding);
    ...
    async checkIn(dto) {
        ...
        let checkInImage;
        if (dto.faceImageBase64) {
            checkInImage = await this.saveFaceImage(dto.employeeId, dto.faceImageBase64, `checkin-${Date.now()}`);
        }
        if (existing) {
    ...
    async checkOut(dto) {
        ...
        let checkOutImage;
        if (dto.faceImageBase64) {
            checkOutImage = await this.saveFaceImage(dto.employeeId, dto.faceImageBase64, `checkout-${Date.now()}`);
        }
        return prisma.attendance.update({
    ...
    async getMonthlyAttendance(employeeId, year, month) {
        ...
        const joinDay = employee?.joinDate
            ? new Date(employee.joinDate.getFullYear(), employee.joinDate.getMonth(), employee.joinDate.getDate())
            : null;
        // Effective start day within the month (1-based)
        const effectiveStartDay = joinDay && joinDay > monthStart ? joinDay.getDate() : 1;
        // Don't count future days
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const effectiveEndDay = Math.min(daysInMonth, today.getFullYear() === year && today.getMonth() === month - 1
            ? today.getDate()
            : daysInMonth);
    ...
    // Helper Methods
    // ────────────────────────────────────────────────────────────────────
    async saveFaceImage(employeeId, base64Image, suffix) {
     ...
     */
    async processImageForComparison(base64Image) {
        const base64Data = base64Image.replace(/^data:image\/\w+;base64,/, "");
        const buffer = Buffer.from(base64Data, "base64");
        return sharp(buffer)
     ...
     */
    async extractSimpleFaceFeatures(imageBuffer) {
